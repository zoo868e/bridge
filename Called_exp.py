...
def quarter(lb, ub):
    times = int((ub - lb) * 4 + 1)
    return [0.25 * i + lb for i in range(times)]

def half(lb, ub):
    times = int((ub - lb) * 2 + 1)
    return [0.5 * i + lb for i in range(times)]

def one_tenth(lb, ub):
    times = int((ub - lb) * 10 + 1)
    return [0.1 * i + lb for i in range(times)]

def one(lb, ub):
    times = ub - lb + 1
    return [1 * i + lb for i in range(times)]
...
#    b = [quarter(0, 4) for i in range(formulaPara[formulaID - 1])]
    b = [one_tenth(-4, 4) for i in range(formulaPara[formulaID - 1])]
#   for train the argument
    best = 0
    for i in range(1, 6):
        makeDATASET(data, i * 1000)
        print("--------------------------------")
