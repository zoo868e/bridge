...
def GA(scorer, lower_bound, upper_bound, dim, popSize, generation, crossover_rate, mutation_rate, elitism_keep, train_process):
    ...
    -------
    best_score: float
        The best fitness after train by GA
    best_ind; list
        The individual that perform the best fitness
    convergence_curve: list
        The convergence curve during the training
    """
    ub = []
    lb = []
    convergence_curve = []
...
#        print("Iter:", l, "the best score is", best_score)
    ...
    return [round(best_score, 3), [round(x, 3) for x in best_ind], convergence_curve]
...
#        individuals[i] = np.clip(individuals[i], lb, ub)
        scores[i] = objf(process, individuals[i, :])
        if scores[i] == 3:
            print(individuals[i, :])
