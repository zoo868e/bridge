...
def mutation(offspring, individualLength, lb, ub):
    ...
    mutationValue = random.randint(lb[mutationIndex], ub[mutationIndex]) / 4
...
def clearDups(Population, lb, ub):
    ...
    if newLen < oldLen:
        ...
        newPopulation = numpy.append(newPopulation, numpy.random.randint(lb, ub, (nDuplicates, len(Population[0]))) / (numpy.array(4)), axis=0)
...
#        ga[i] = [*random.choice(scorematrix)[1][0], *random.choice(scorematrix)[2][0], *z]
    for i in range(dim):
        ga[:, i] = numpy.random.randint(lb[i], ub[i]) / 4
